...
const MEDIA_CONC = 8;
const GROUP_CONC = Number(process.env.GROUP_CONC || 8);  // было 30 (краш 27-го) → 3 (щадяще, но ~4ч) → 8: узкое место = латентность round-trip'ов (Firestore/RTDB/PB-проверки), не SQLite-writer; батч 50 короткий, PB держит. Следим за health, откат при деградации.
const CHUNK_PAUSE_MS = 80; // пауза между пачками — дать writer'у выдохнуть.
...
const SKIP_EXISTING = process.env.NO_SKIP === '1' ? new Set() : new Set(['memories', 'memory_comments', 'canvas_strokes', 'chat_messages']);
...
async function authPb() {
  const r = await pb('POST', '/api/collections/_superusers/auth-with-password',
    { identity: process.env.PB_EMAIL || 'badzoff@gmail.com', password: PB_PW });
...
(async () => {
  ...
  if (ONLY_GROUP) {
    ...
    // Resume: пропускаем уже обработанные группы (записанные в DONE_FILE на
    // прошлых прогонах/после аборта по нагрузке) — чтобы не переделывать.
    const fs = require('fs');
    const DONE_FILE = process.env.DONE_FILE || '/tmp/mig_done.txt';
    const doneSet = new Set(
      fs.existsSync(DONE_FILE) ? fs.readFileSync(DONE_FILE, 'utf8').split('\n').filter(Boolean) : []
    );
    const gsnap = await db.collection('groups').select().get();
    const gidArr = gsnap.docs
      .filter((g) => g.updateTime && g.updateTime.toMillis() >= SINCE_MS)
      .map((g) => g.id)
      .filter((id) => !doneSet.has(id));
    console.log(`Групп к обходу: ${gidArr.length} (пропущено готовых: ${doneSet.size})`);
    let done = 0; const t0 = Date.now();
    await pMap(gidArr, async (gid) => {
      await deltaGroup(gid);
      fs.appendFileSync(DONE_FILE, gid + '\n');
      done++;
