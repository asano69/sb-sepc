...
async function drawPot(_potData) {
  ...
  for (let i = 0; i < edgePoints.length - 1; i++) {
    if (isOutCurve) edgeCurves.push(curvesOut[int(random(curvesOut.length))]);
    else edgeCurves.push(curvesIn[int(random(curvesIn.length))]);
  ...
  let baseY =
    startY + baseRadius * 0.24 * -cos(radians(lerp(90, 270, baseAngleT)));

  let endX = startX + endRadius * sin(radians(lerp(90, 270, endAngleT)));
  let endY =
    startY -
    endYOffset +
    endRadius * 0.24 * -cos(radians(lerp(90, 270, endAngleT)));
...
function drawStickBranch(_x1, _y1, _x2, _y2, _thickness) {
  ...
  for (let i = 0; i < dotCount; i++) {
    ...
    nowX +=
      sin(radians(normalAngle)) *
      noise(nowX * 0.1, nowY * 0.1, 666.0) *
      _thickness;
    nowY -=
      cos(radians(normalAngle)) *
      noise(nowX * 0.1, nowY * 0.1, 999.0) *
      _thickness;
